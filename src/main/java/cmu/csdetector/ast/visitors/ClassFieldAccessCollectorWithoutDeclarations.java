 ...
 * @author pallav
 */
public class ClassFieldAccessCollectorWithoutDeclarations extends CollectorVisitor<IVariableBinding> {
	...
	private Set<String> allVariablesNames;

	public ClassFieldAccessCollectorWithoutDeclarations(TypeDeclaration declaringType) {
		this.declaringTypeBinding = declaringType.resolveBinding();
		this.allVariables = this.getVariablesInHierarchy();
		this.allVariablesNames = this.getVariableNames();
	...
	public boolean visit(SimpleName node) {
		...
		*/
		...
		if (binding.getKind() == IBinding.VARIABLE) {
			...
			if (!wasAlreadyCollected(variableBinding) &&
					(this.allVariables.contains(variableBinding) || this.allVariablesNames.contains(variableBinding.getName()))){
	...
	private Set<String> getVariableNames(){
		...
		for(IVariableBinding v: this.allVariables){
			names.add(v.getName());
		}
		return names;
	}
